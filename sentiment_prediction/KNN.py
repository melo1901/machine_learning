...
# Defniuj model (k-sąsiadów)
knn_model = KNeighborsClassifier(n_neighbors=5)  # Wybrałem liczbę najbliższych sąsiadów równą 5
...
# Policz obszar pod krzywą ROC
roc_auc = roc_auc_score(y_true_binary, knn_model.predict_proba(X_test_vec)[:, 1])
...
# Generuj krzywą ROC
fpr, tpr, thresholds = roc_curve(y_true_binary, knn_model.predict_proba(X_test_vec)[:, 1])
...
# Zapisz model i vectorizer do pliku
#joblib.dump((knn_model, vectorizer), 'model_and_vectorizerKNN.joblib')